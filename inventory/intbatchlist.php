...
<head>
    ...
    <title>Ingredient Batch List</title>
</head>
<body>
    <div class="main">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped table-hover">
                                        ...
                                        <tbody>
                                            <?php
                                            $today = date("Y-m-d");
                                            $twoweeks = date("Y-m-d", strtotime("+7 days"));

                                            $sql = "SELECT intbatch.*, inventory.IN_NAME
                                                    FROM intbatch
                                                    JOIN inventory ON inventory.IN_ID = intbatch.IN_ID
                                                    ORDER BY intbatch.EXP_DATE ASC";
                                            $query = mysqli_query($sqlc, $sql);
                                            
                                            if(mysqli_num_rows($query)):?>
                                                <?php while($row = mysqli_fetch_assoc($query)): 
                                                    $exdate = $row['EXP_DATE'];
                                                    $warning = ($exdate <= $twoweeks) ? "style='color: red; font-weight: bold;'": "";    
                                                ?>
                                                    <tr <?= $warning?>>
                                                        <td><?= htmlspecialchars($row['B_ID'])?></td>
                                                        <td><?= htmlspecialchars($row['IN_NAME'])?></td>
                                                        <td><?= htmlspecialchars($row['BATCH_Q'])?></td>
                                                        <td><?= htmlspecialchars($row['EXP_DATE'])?></td>
                                                        <td>
                                                            <a href="delfunc.php?IBDEL=<?= $row['B_ID']?>" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this?');"><i class="bi bi-trash"></i></a>
                                                        </td>
                                                    </tr>
                                                <?php endwhile; ?>
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="8">
                                                        <center>No Batch Available</center>
                                                    </td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
